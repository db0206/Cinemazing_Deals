...
import { setCustomerCoupons } from "../../redux/usersReducer";
import "../Coupon.css";
...
const CustomerCouponList = () => {
  ...
  const coupons = useSelector((state) => state.users.customerCoupons);
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/customer/coupons", {
        headers: {
          Authorization: token,
        },
      })
      .then((response) => dispatch(setCustomerCoupons(response.data)))
      .catch((error) => {
  ...
  useEffect(() => {
    if (couponCategory !== Category.EMPTY) {
      axios
        .get(
          `http://localhost:8080/customer/coupons-category?category=${couponCategory}`,
          {
            headers: {
              Authorization: token,
            },
  ...
  const callCouponsByPrice = (maxPrice) => {
    axios
      .get(`http://localhost:8080/customer/coupons-price?price=${maxPrice}`, {
        headers: {
          Authorization: token,
        },
      })
